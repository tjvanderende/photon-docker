...
| `FILE_URL`             | URL to a .tar.bz2/.jsonl.zst file      | -                                | Set a custom URL for the index file to be downloaded. Supports HTTP(S) URLs (e.g., "https://download1.graphhopper.com/public/experimental/photon-db-latest.tar.bz2") and S3 URLs (e.g., "s3://my-bucket/photon-db-latest.tar.bz2"). Supports `.tar.bz2` and `.jsonl.zst` formats. Setting this overrides `UPDATE_STRATEGY` to `DISABLED`, and `SKIP_MD5_CHECK` to true if `MD5_URL` is not set. For S3, configure credentials via `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` env vars or IAM roles. Use `AWS_ENDPOINT_URL` or `S3_ENDPOINT_URL` for S3-compatible services (e.g., MinIO, R2). |
